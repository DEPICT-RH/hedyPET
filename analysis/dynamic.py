...
if not aorta_vois_path.exists() or not if_tac_path.exists():
    print("Extracting TAC")
    aorta_seg = totalseg.get_fdata()==52
    aorta_vois, aorta_segments = extract_aorta_vois(aorta_seg, totalseg.affine, dpet, frame_time_start, t_threshold=40, volume_ml=CYLINDER_VOLUME_ML,cylinder_width=CYLINDER_WIDTH_PX)
    if_tac = extract_tac(dpet, aorta_vois==AortaSegment.DESCENDING_BOTTOM.value)
...
seg[~np.isin(seg,[90,1,2,3,4,5,6,7,15,16,20,22,52])] = 0
...
def run_save_patlak(tac,if_tac,frame_time_middle, out_dir, n_frames_regression=4,region_name="brain",save_img=False):
...
run_save_patlak(brain_tac,if_tac,frame_time_middle,WORKING_DIR / sub,save_img=True)
