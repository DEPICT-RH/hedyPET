...
from hedypet.utils import RAW_ROOT, load_sidecar
import pandas as pd
...
with open(f"{os.path.dirname(__file__)}/synthseg_classes.json","r") as handle:
    SYNTHSEG_CLASS_TO_LABEL  = json.load(handle)
...
def plot_patplak(X,Y, slope, intercept, n_frames_regression):
    ...
    plt.plot(xs,reg(xs),'k',linewidth=1,label=f"Ki={slope:.2E},b={intercept:.2E}")
    plt.plot(X,Y,'.r')
    plt.ylim(np.nanmin(Y),np.nanmax(Y[-n_frames_regression:])*1.1)


def get_t(sub):
    return np.loadtxt(f"/depict/users/hinge/private/hedypet/dynamic/{sub}/t_tac.txt")
